...
export type PersistedLocation = Pick<
  WeatherLocation,
...
export function getPersistedLocation(
  ...
  try {
    ...
    return isPersistedLocation(parsed) ? parsed : null
  } catch {
    return null
  }
}
...
export function persistLocation(
  ...
  try {
    storage.setItem(LAST_LOCATION_STORAGE_KEY, JSON.stringify(value))
  } catch {
...
export function formatLocationLabel(
  location: Pick<WeatherLocation, 'name' | 'state' | 'country'>,
): string {

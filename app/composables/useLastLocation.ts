...
export function useLastLocation() {
  ...
  function restore() {
    ...
    location.value = getPersistedLocation()
    return location.value
  }

  function save(weatherLocation: WeatherLocation) {
    if (!import.meta.client) return
    persistLocation(weatherLocation)
    location.value = {
      name: weatherLocation.name,
      country: weatherLocation.country,
      ...(weatherLocation.state ? { state: weatherLocation.state } : {}),

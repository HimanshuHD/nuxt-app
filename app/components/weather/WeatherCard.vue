...
function weatherIconUrl(icon: string) { return `https://openweathermap.org/img/wn/${icon}@2x.png` }
function countryFlagUrl(country: string) { return `https://flagcdn.com/w80/${country.toLowerCase()}.png` }
function formatTemperature(value: number) { return `${Math.round(value)}°` }
</script>

<template>
  <section class="weather-card weather-card--stable" aria-live="polite" :aria-busy="props.loading">
    <p v-if="props.loading" class="sr-only" aria-live="polite">Loading weather data</p>
      ...
      <div class="weather-main">
        <div class="weather-location">
          ...
          <div class="location-title">
            <img class="country-flag" :src="countryFlagUrl(props.weather.location.country)" :alt="`${props.weather.location.country} flag`" width="40" height="27" loading="lazy">
        ...
        <div class="weather-visual-wrap">
          <WeatherVisual :condition="props.weather.description" :icon="props.weather.icon" />
          <div class="temperature">
            <img :src="weatherIconUrl(props.weather.icon)" alt="" aria-hidden="true">
            <strong>{{ formatTemperature(props.weather.temperature) }}</strong>
          </div>
        </div>
      </div>
      <div class="weather-summary">
        <div><span>Feels like</span><strong>{{ formatTemperature(props.weather.feelsLike) }}</strong></div>
        <div><span>Humidity</span><strong>{{ props.weather.humidity }}%</strong></div>
        <div><span>Wind</span><strong>{{ props.weather.windSpeed }} m/s</strong></div>
        <div><span>Pressure</span><strong>{{ props.weather.pressure }} hPa</strong></div>
        <div><span>Visibility</span><strong>{{ (props.weather.visibility / 1000).toFixed(1) }} km</strong></div>
        <div><span>Cloudiness</span><strong>{{ props.weather.cloudiness }}%</strong></div>
        <div><span>Sunrise</span><strong>{{ props.weather.sunrise }}</strong></div>
        <div><span>Sunset</span><strong>{{ props.weather.sunset }}</strong></div>
      </div>
      <p class="weather-meta">Coordinates: {{ props.weather.location.latitude.toFixed(4) }}, {{ props.weather.location.longitude.toFixed(4) }}</p>
    </template>
...
<style scoped>
.weather-card--stable { min-height: 430px; position: relative; z-index: 1; }
.weather-location { min-width: 0; }
.location-title { display: flex; align-items: center; gap: .75rem; }
.location-title h2 { margin: 0; font-size: clamp(1.8rem, 4vw, 2.75rem); }
.country-flag { width: 40px; height: 27px; flex: 0 0 auto; object-fit: cover; border-radius: 4px; box-shadow: 0 4px 12px rgba(0,0,0,.2); }
.weather-visual-wrap { display: flex; align-items: center; gap: .5rem; }
.weather-visual-wrap .temperature { margin-top: .5rem; }
